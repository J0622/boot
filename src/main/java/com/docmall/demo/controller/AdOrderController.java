...
public class AdOrderController {
	@Value("${file.dir}") // servlet-context.xml에서 bean이름 참조
	private String uploadPath;
	
	private final AdOrderService adOrderService;

	@GetMapping("/order_list")
	public void order_list(Criteria cri, @ModelAttribute("end_date") String end_date,
			@ModelAttribute("start_date") String start_date, Model model) throws Exception {
...
//	주문상세 방법 1) 주문상세정보가 클라이언트 json 형태로 변환되어 보내진다. (pom.xml 에 jackson-databind 라이브러리 백그라운드로 작동)
	...
	public ResponseEntity<List<OrderDetailInfoVO>> order_detail_list1(@PathVariable("ord_code") Long ord_code)
			throws Exception {
